...
#
# Tags look like `v<ntfy version>-<release>`:
#
# - if defaults/main.yml points at an ntfy version that has never been
#   released, the release counter restarts at 0 (`v2.27.0-0`)
# - otherwise the counter is incremented (`v2.27.0-1`), but only if something
...
#
# `vars` is in this list because this role keeps computed values there
# (the `/metrics` port, the request-limit exemption list), which end up in the
# rendered container labels and environment file just like `defaults` does.
role_defining_paths=(
	'defaults'
	'meta'
	'tasks'
...
# Only the leaf assignment is read. Values derived from it
# (`ntfy_container_image`, which interpolates `{{ ntfy_version }}`) are not
# anchored at the start of a line and so cannot be picked up by accident.
version="$(sed -nE 's|^ntfy_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the ntfy version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v2.27.0`),
